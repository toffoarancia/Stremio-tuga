...
// Stream proxy — fetches m3u8 with correct headers and serves to Stremio
app.get('/proxy/stream', async (req, res) => {
  const url = req.query.url;
  const referer = req.query.ref || 'https://minochinos.com/';
  if (!url) return res.status(400).send('Missing url');
  try {
    ...
    res.setHeader('Content-Type', resp.headers.get('content-type') || 'application/vnd.apple.mpegurl');
