...
async function extractDirectVideo(playerUrl) {
  ...
  try {
    ...
    page.on('request', req => {
      ...
      if (['image', 'font', 'media'].includes(rt)) return req.abort();
      if (['googlesyndication','doubleclick','amazon-adsystem'].some(h => u.includes(h))) return req.abort();
      if (u.match(/\.(mp4|m3u8|mkv|webm)(\?|$)/i)) {
        foundUrl = u;
        foundReferer = req.headers()['referer'] || playerUrl;
      }
      req.continue();
    });
    page.on('response', resp => {
      ...
      // Catch m3u8 URLs that don't end with .m3u8 but contain it
      else if (u.includes('.m3u8') && u.startsWith('http')) foundUrl = u;
    });
    await page.setUserAgent('Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 Chrome/124.0.0.0 Safari/537.36');
    ...
    await page.evaluate(() => {
      const play = document.querySelector('button.play, .play-button, [class*="play"], video');
      if (play) play.click();
    }).catch(() => {});
    ...
    console.log('[directVideo] ' + playerUrl + ' -> ' + (foundUrl || 'not found') + ' referer=' + foundReferer);
  } catch(e) {
    console.error('[directVideo] error: ' + e.message);
  } finally {
    await page.close().catch(()=>{});
  }
  return foundUrl ? { url: foundUrl, referer: foundReferer } : null;
}
...
// Stream proxy — uses Puppeteer to get cookies from embed page then fetches m3u8
const streamCache = new Map(); // embedUrl -> { m3u8Url, cookies, referer, ts }

app.get('/proxy/stream', async (req, res) => {
  const embedUrl = req.query.ref;
  const m3u8Url = req.query.url;
  if (!m3u8Url) return res.status(400).send('Missing url');

  try {
    // Get cookies by loading the embed page in Puppeteer
    let cookies = '';
    ...
    if (cached && Date.now() - cached.ts < 300000) {
      ...
      try {
        ...
        streamCache.set(embedUrl, { cookies, ts: Date.now() });
      } finally {
        await page.close().catch(() => {});
      }
    }
    ...
    const headers = {
      'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 Chrome/124.0.0.0 Safari/537.36',
      'Referer': embedUrl || 'https://minochinos.com/',
      'Origin': embedUrl ? new URL(embedUrl).origin : 'https://minochinos.com',
    };
    ...
    const text = await resp.text();
    // Rewrite relative segment URLs to go through our proxy too
    ...
    const rewritten = text.replace(/^(?!#)(.+)$/gm, line => {
      ...
      // Proxy segment URLs too
      return ADDON_URL + '/proxy/segment?url=' + encodeURIComponent(absUrl) + '&ref=' + encodeURIComponent(embedUrl || '');
    });
    res.send(rewritten);
  } catch(e) {
...
// Proxy for individual HLS segments
...
// Live stream endpoint - extracts fresh stream URL on demand (called by Stremio at playback time)
app.get('/live-stream', async (req, res) => {
  const embedUrl = req.query.embed;
  if (!embedUrl) return res.status(400).send('Missing embed URL');
  console.log('[live-stream] extracting from embed: ' + embedUrl);
  try {
    const result = await extractDirectVideo(embedUrl);
    if (!result) return res.status(404).send('No stream found');
    
    // Fetch the m3u8 immediately and proxy it
    const resp = await fetch(result.url, {
      headers: {
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 Chrome/124.0.0.0 Safari/537.36',
        'Referer': result.referer || embedUrl,
        'Origin': (() => { try { return new URL(result.referer || embedUrl).origin; } catch { return ''; } })(),
      }
    });
    if (!resp.ok) return res.status(resp.status).send('Stream fetch failed: ' + resp.status);
    ...
    // Proxy segments through our server
    const rewritten = text.replace(/^(?!#)(.+)$/gm, line => {
      line = line.trim();
      if (!line) return line;
      const absUrl = line.startsWith('http') ? line : new URL(line, base).href;
      return ADDON_URL + '/proxy/segment?url=' + encodeURIComponent(absUrl) + '&ref=' + encodeURIComponent(result.referer || embedUrl);
    });
    res.send(rewritten);
  } catch(e) {
