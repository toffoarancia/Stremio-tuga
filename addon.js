      ...
      try {
        ...
        if (data.embed_url) {
          ...
          if (m && m[1].startsWith('http')) {
            ...
            // For non-minochinos players, try direct stream extraction
            // minochinos has adblocker detection so skip live-stream for it
            if (!embedUrl.includes('minochinos.com')) {
              const liveUrl = ADDON_URL + '/live-stream?embed=' + encodeURIComponent(embedUrl);
              addStream({ title: '▶ Player ' + (i+1) + ' PT (direto)', url: liveUrl });
            }
            // Always add embed as external option
...
async function extractDirectVideo(playerUrl) {
  ...
  return new Promise(async (resolve) => {
    ...
    // Wait 30s — use last stream URL found (real content after ad fails)
    setTimeout(() => {
      const last = allStreamUrls[allStreamUrls.length - 1];
      done(last || (foundUrl ? { url: foundUrl, referer: foundReferer } : null));
    }, 30000);
    ...
    try {
      ...
      page.on('request', req => {
        ...
        if (['image', 'font', 'media'].includes(rt)) return req.abort();
        if (['googlesyndication','doubleclick','amazon-adsystem'].some(h => u.includes(h))) return req.abort();
        if ((u.match(/\.(mp4|m3u8|mkv|webm)(\?|$)/i) || u.includes('.m3u8')) && u.includes('minochinos.com')) {
          const referer = req.headers()['referer'] || playerUrl;
          allStreamUrls.push({ url: u, referer });
          foundUrl = u; foundReferer = referer;
          console.log('[directVideo] stream #' + allStreamUrls.length + ': ' + u.slice(-50));
          const masters = allStreamUrls.filter(s => s.url.includes('master.m3u8'));
          if (masters.length >= 2) {
            console.log('[directVideo] 2nd master = real content!');
            done(masters[masters.length - 1]);
          }
        }
        req.continue();
      });
      ...
      // Bypass adblocker detection before page loads
      await page.evaluateOnNewDocument(() => {
        // Make ads appear to load successfully
        Object.defineProperty(document, 'adblockDetected', { get: () => false, configurable: true });
        // Override the static.js ad check
      ...
      // Check if adblocker was detected
      const adblockMsg = await page.evaluate(() => {
        const body = document.body.innerText || '';
        return body.includes('adblock') || body.includes('blocker') || body.includes('desativar');
      }).catch(() => false);
      console.log('[directVideo] adblock detected by site: ' + adblockMsg);

      // Try to extract content URL directly from JWPlayer config
      const jwConfig = await page.evaluate(() => {
        try {
          if (typeof jwplayer !== 'undefined') {
            const p = jwplayer();
            if (p && p.getPlaylist) return JSON.stringify(p.getPlaylist());
          }
          for (const s of document.querySelectorAll('script:not([src])')) {
            const m = s.textContent.match(/jwplayer[^.]*\.setup\s*\(\s*(\{[\s\S]+?\})\s*\)/);
            if (m) return m[1].slice(0, 2000);
          }
        } catch(e) { return 'error: ' + e.message; }
        return null;
      }).catch(() => null);
      console.log('[directVideo] jwConfig: ' + (jwConfig || 'none').slice(0, 300));

      // Try to skip ads via JWPlayer API, then click play
      await page.evaluate(() => {
        try {
          if (typeof jwplayer !== 'undefined') {
            const p = jwplayer();
            ...
            if (p.play) { p.play(); return; }
          }
        } catch(e) {}
        const selectors = ['.jw-icon-display', '.jw-display-icon-container', 'video', '[aria-label="Play"]'];
      ...
      // Wait 3s then grab the new playlist URL from JWPlayer (different token = real content)
      await new Promise(r => setTimeout(r, 3000));
      const realStreamUrl = await page.evaluate(() => {
        try {
      ...
      if (realStreamUrl) {
        console.log('[directVideo] got real stream from JWPlayer: ' + realStreamUrl.slice(-60));
        // Fetch child m3u8 to check if it has real content (not TikTok ads)
        const childCheck = await page.evaluate(async (masterUrl, ref) => {
          try {
            const base = masterUrl.replace('master.m3u8', '');
            const childUrl = base + 'index-v1-a1.m3u8';
            const r = await fetch(childUrl, { headers: { 'Referer': ref } });
            const text = await r.text();
            const firstSeg = text.split('\n').find(l => l.trim() && !l.startsWith('#')) || '';
            return { firstSeg, isAd: firstSeg.includes('tiktokcdn') };
          } catch(e) { return { error: e.message }; }
        }, realStreamUrl, playerUrl).catch(() => null);

        console.log('[directVideo] child check: ' + JSON.stringify(childCheck));

        if (childCheck && !childCheck.isAd) {
          done({ url: realStreamUrl, referer: playerUrl });
          return;
        }
        // Even if child is still ad, use this URL — it's the best we have
        done({ url: realStreamUrl, referer: playerUrl });
        return;
      }

      // Wait up to 20s for real stream via request interception
      ...
      done(foundUrl ? { url: foundUrl, referer: foundReferer } : null);
    } catch(e) {
...
app.get('/test-embed', async (req, res) => {
  ...
  try {
    ...
    // Fetch child m3u8 to check content
    ...
    } catch(e) { childFirst = 'fetch error: ' + e.message; }
    res.json({
      streamUrl: result.url,
      referer: result.referer,
      childUrl,
      isAd: childFirst.includes('tiktokcdn'),
      childFirstSegments: childFirst,
    });
  } catch(e) {
    res.json({ error: e.message });
  }
...
app.get('/proxy/m3u8', async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      const rewritten = result.text.replace(/^(?!#)(.+)$/gm, line => {
        line = line.trim();
        if (!line) return line;
        const absUrl = line.startsWith('http') ? line : new URL(line, base).href;
        return ADDON_URL + '/proxy/seg?url=' + encodeURIComponent(absUrl)
