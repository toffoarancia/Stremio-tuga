...
// Segment proxy: uses Puppeteer to fetch segments (bypasses Railway DNS blocks)
  ...
  try {
    ...
    try {
      const result = await page.evaluate(async (url, referer, origin) => {
        ...
        return { data: arr, type: resp.headers.get('content-type') || 'video/mp2t' };
      }, url, referer, origin);
      if (result.error) return res.status(result.error).send('Segment fetch failed');
