  ...
  return new Promise(async (resolve) => {
    ...
    try {
      ...
      page.on('request', req => {
        ...
        if (['image', 'font', 'media'].includes(rt)) return req.abort();
        // Block ad networks including TikTok ads and minochinos ad endpoint
        ...
        // Block minochinos ad request specifically
        ...
        if ((u.match(/\.(mp4|m3u8|mkv|webm)(\?|$)/i) || u.includes('.m3u8')) && u.includes('minochinos.com')) {
          ...
          console.log('[directVideo] found minochinos stream: ' + u);
      ...
      page.on('response', resp => {
        ...
        if ((u.match(/\.(mp4|m3u8|mkv|webm)(\?|$)/i) || u.includes('.m3u8')) && u.includes('minochinos.com')) {
          foundUrl = u;
          console.log('[directVideo] found minochinos stream via response: ' + u);
          done({ url: foundUrl, referer: foundReferer });
        }
      });
      await page.setUserAgent('Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 Chrome/124.0.0.0 Safari/537.36');
      await page.goto(playerUrl, { waitUntil: 'domcontentloaded', timeout: 15000 });
      await new Promise(r => setTimeout(r, 8000)); // Wait for ads to finish
      if (!foundUrl) {
        // Try clicking play
        await page.evaluate(() => {
