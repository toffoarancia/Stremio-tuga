    ...
    for (let i = 0; i < Math.min(pageInfo.tabs.length, 3); i++) {
      ...
      try {
        ...
        if (data.embed_url) {
          ...
          if (m && m[1].startsWith('http')) {
            const embedUrl = m[1];
            const directResult = await extractDirectVideo(embedUrl).catch(() => null);
            if (directResult) {
              addStream({
                title: '▶ Player ' + (i+1) + ' PT (direto)',
                url: directResult.url,
                behaviorHints: {
                  notWebReady: false,
                  headers: {
                    'Referer': directResult.referer || embedUrl,
                    'Origin': (() => { try { return new URL(directResult.referer || embedUrl).origin; } catch { return ''; } })(),
                  }
                }
              });
            }
            // Always add embed URL as fallback
            try {
              const host = new URL(embedUrl).hostname.replace('www.', '');
