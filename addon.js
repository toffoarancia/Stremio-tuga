      ...
      try {
        ...
        if (embedUrl && embedUrl.startsWith('http')) {
          ...
          if (direct && direct.url) {
            ...
            streams.push({ title: '▶ PT (direto)', url: liveUrl });
          } else {
            try {
  ...
  return new Promise(async (resolve) => {
    ...
    try {
      ...
      await page.goto(playerUrl, { waitUntil: 'networkidle2', timeout: 20000 }).catch(() =>
        page.goto(playerUrl, { waitUntil: 'domcontentloaded', timeout: 15000 })
      );
...
async function extractStreamsWithPuppeteer(pageUrl) {
  ...
  try {
    ...
    for (const src of allSrcs) {
      ...
      if (direct && direct.url) {
        const liveUrl = ADDON_URL + '/live-stream?embed=' + encodeURIComponent(src);
        addStream({ title: '▶ PT (direto)', url: liveUrl });
      } else {
...
app.get('/debug-player', async (req, res) => {
  const embedUrl = req.query.embed || 'https://osteusfilmestuga.upns.online/#msno8a';
  const br = await getBrowser();
  const page = await br.newPage();
  const requests = [];
  try {
    ...
    page.on('request', req => {
      ...
      requests.push({ url: u.slice(0, 150) });
      req.continue();
    });
    await page.setUserAgent('Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 Chrome/124.0.0.0 Safari/537.36');
    await page.goto(embedUrl, { waitUntil: 'networkidle2', timeout: 20000 });
    await new Promise(r => setTimeout(r, 5000));
    const streams = requests.filter(r => r.url.match(/\.(m3u8|mp4|ts)(\?|$)/i) || r.url.includes('.m3u8'));
    const html = (await page.content()).slice(0, 2000);
    res.json({ embedUrl, streams, totalRequests: requests.length, htmlSnippet: html });
  } catch(e) {
    res.json({ error: e.message, requests: requests.slice(0, 20) });
  } finally {
    await page.close().catch(() => {});
  }
});
  ...
  try {
    ...
    for (let i = 0; i < 6; i++) {
      ...
      try {
        ...
        if (!firstSeg.includes('tiktokcdn')) break;
...
app.get('/live-stream', async (req, res) => {
  ...
  try {
    ...
    const embedOrigin = (() => { try { return new URL(embedUrl).origin; } catch { return ''; } })();
    const origin = (() => { try { return new URL(referer).origin; } catch { return embedOrigin || 'https://minochinos.com'; } })();
