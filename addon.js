...
async function extractDirectVideo(playerUrl) {
  ...
  return new Promise(async (resolve) => {
    ...
    try {
      ...
      // Wait up to 20s for real stream (after ad m3u8, player should switch to content)
...
app.get('/debug-m3u8', async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      // Wait 10s then re-fetch child m3u8 — check if real content loads after ad blocked
      await new Promise(r => setTimeout(r, 10000));
      let childAfterWait = null;
      if (m3u8Url && !page.isClosed()) {
        const childUrl = m3u8Url.replace('master.m3u8', 'index-v1-a1.m3u8');
        ...
        childAfterWait = { url: childUrl, ...childResp };
      }
