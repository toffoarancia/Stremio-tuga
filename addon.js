  ...
  return new Promise(async (resolve) => {
    ...
    try {
      ...
      page.on('request', req => {
        ...
        if (['image', 'font', 'media'].includes(rt)) return req.abort();
        // Block TikTok/ads — causes player to error and switch to real content
        const adHosts = ['googlesyndication','doubleclick','amazon-adsystem','tiktokcdn','tiktok.com','imasdk.googleapis','googleads','adnxs','taboola','outbrain','yandex.com','mc.yandex'];
        ...
        if ((u.match(/\.(mp4|m3u8|mkv|webm)(\?|$)/i) || u.includes('.m3u8')) && u.includes('minochinos.com')) {
          ...
          // 2nd master.m3u8 = real content after ad fails
      ...
      await new Promise(r => setTimeout(r, 2000));
      // Click play immediately — this triggers the real stream after ads
      await page.evaluate(() => {
        // JWPlayer play button or video element
        const selectors = ['.jw-icon-display', '.jw-display-icon-container', 'video', '.play-button', '[aria-label="Play"]'];
