...
async function extractDirectVideo(playerUrl) {
  ...
  return new Promise(async (resolve) => {
    ...
    // Set a hard timeout of 25 seconds — need to wait for ads to finish
    setTimeout(() => done(foundUrl ? { url: foundUrl, referer: foundReferer } : null), 25000);
    ...
    try {
      ...
      page.on('request', req => {
        ...
        if (['image', 'font', 'media'].includes(rt)) return req.abort();
        if (['googlesyndication','doubleclick','amazon-adsystem'].some(h => u.includes(h))) return req.abort();
        if (u.match(/\.(mp4|m3u8|mkv|webm)(\?|$)/i) || u.includes('.m3u8')) {
          // Ignore ad networks
          if (u.includes('tiktokcdn') || u.includes('doubleclick') || u.includes('googlesyndication') || u.includes('ad-site')) return req.continue();
          foundUrl = u;
          foundReferer = req.headers()['referer'] || playerUrl;
          console.log('[directVideo] found via request: ' + u);
          done({ url: foundUrl, referer: foundReferer });
        }
        req.continue();
      });
      page.on('response', resp => {
        const u = resp.url();
        if (u.includes('tiktokcdn') || u.includes('ad-site') || u.includes('doubleclick') || u.includes('googlesyndication')) return;
        if (u.match(/\.(mp4|m3u8|mkv|webm)(\?|$)/i) || u.includes('.m3u8')) {
          if (!foundUrl) {
            foundUrl = u;
            console.log('[directVideo] found via response: ' + u);
            done({ url: foundUrl, referer: foundReferer });
          }
        }
      });
...
app.get('/debug-child-m3u8', async (req, res) => {
  ...
  try {
    ...
    // Fetch master m3u8
    const master = await page.evaluate(async (url, referer, origin) => {
      const r = await fetch(url, { headers: { 'Referer': referer, 'Origin': origin } });
      return { status: r.status, text: await r.text() };
    }, result.url, referer, origin);
    // Find child m3u8 URL
    const base = new URL(result.url);
    const childRelative = master.text.split('\n').find(l => l.trim() && !l.startsWith('#'));
    if (!childRelative) return res.json({ master: master.text, error: 'No child found' });
    const childUrl = childRelative.startsWith('http') ? childRelative : new URL(childRelative.trim(), base).href;
    // Fetch child m3u8
    const child = await page.evaluate(async (url, referer, origin) => {
      const r = await fetch(url, { headers: { 'Referer': referer, 'Origin': origin } });
      return { status: r.status, text: await r.text() };
    }, childUrl, referer, origin);
    res.json({ masterUrl: result.url, childUrl, masterContent: master.text, childContent: child.text });
  } catch(e) {
    res.json({ error: e.message });
  }
});
