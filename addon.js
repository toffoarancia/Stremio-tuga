...
app.get('/live-stream', async (req, res) => {
  ...
  try {
    ...
    if (!resp.ok) return res.status(resp.status).send('m3u8 fetch failed: ' + resp.status);
    ...
    // Check if this is a master playlist (contains .m3u8 references)
    // or a media playlist (contains .ts segments)
    const rewritten = text.replace(/^(?!#)(.+)$/gm, line => {
      ...
      if (absUrl.includes('.m3u8') || absUrl.includes('.m3u8?')) {
        // Child playlist — proxy through live-stream with same referer
...
// Proxy child m3u8 playlists (from master playlist)
app.get('/proxy/m3u8', async (req, res) => {
  ...
  try {
    ...
    if (!resp.ok) return res.status(resp.status).send('Child m3u8 failed: ' + resp.status);
    res.setHeader('Content-Type', 'application/vnd.apple.mpegurl');
    res.setHeader('Access-Control-Allow-Origin', '*');
    res.setHeader('Cache-Control', 'no-cache');
    const text = await resp.text();
...
// Segment proxy: pipes individual .ts segments with correct headers
