      ...
      try {
        ...
        if (data.embed_url) {
          ...
          if (m && m[1].startsWith('http')) {
            ...
            const direct = await extractDirectVideo(embedUrl).catch(() => null);
            if (direct) {
              // Proxy the stream through our server to add correct Referer header
              const proxyUrl = ADDON_URL + '/proxy/stream?url=' + encodeURIComponent(direct) + '&ref=' + encodeURIComponent(embedUrl);
              addStream({
